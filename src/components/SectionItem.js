...
const { AiOutlineHeart, BsFillPlayFill, BsThreeDotsVertical } = icons
        ...
        >
            ...
            >
                {isHover && <div className='absolute top-0 bottom-0 z-40 left-0 right-0 bg-overlay-30 rounded-lg text-white flex items-center justify-center gap-3'>
                    ...
                    <span><BsThreeDotsVertical size={25} /></span>
                </div>}
            ...
            <span className='flex flex-col'>
                ...
                {data?.sectionId === 'h100' ? <span>{artistsNames}</span> : <span>{sortDescription?.length >= 30 ? `${sortDescription?.slice(0, 30)}...` : sortDescription}</span>}
            </span>
        </div>
    )
}
